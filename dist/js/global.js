...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__modules_sensor__ = __webpack_require__(334);
...
Object(__WEBPACK_IMPORTED_MODULE_1__libs_binder__["a" /* binder */])({
    bounds: {
        ".sensor": __WEBPACK_IMPORTED_MODULE_2__modules_sensor__["a" /* sensorsInit */]
    },
    runTests: false
});
...
function sensorsInit() {
    ...
    function successAnimation(input, dot, value, holder, time) {
        ...
        var rotateControlPoint = -135 + 270 / 100 * value.percent();
        var inputControlPoint = value.current;
        var inputTiming = time / (Math.abs(value.min) + value.current);
        var rotateTiming = time / (rotateControlPoint - 135);
    ...
    sensor.forEach(function (item) {
        var some = item;
        var dot = some.querySelector(".dot");
        var indicator = dot.querySelector(".indicator");
        var input = some.querySelector("input");
        var valueHolder = some.querySelector(".value-holder span");
        var time = 500;
        ...
        var value = {
            ...
            percent: function percent() {
                return (Math.abs(value.min) + value.current) / value.fullRange() * 100;
            }
        };
        ...
        input.addEventListener('input', function () {
            var percent = (Math.abs(value.min) + parseInt(this.value)) / value.fullRange() * 100;
